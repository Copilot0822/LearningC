...
void demo_scanf_basics(void) {
    ...
    printf("Enter: <int> <double> <word>\nExample: 10 3.14 hello\n> ");
    int rc = scanf("%d %lf %15s", &a, &b, w);
    printf("scanf returned %d\n", rc);
    if (rc == 3) {
        printf("Parsed: a=%d b=%.3f w=%s\n", a, b, w);
    } else {
...
void demo_mixed_io(void) {
    ...
    int id; double score; char grade; char name[32];
    printf("Enter: <id:int> <score:double> <grade:char> <name:word>\n");
    printf("Example: 123 87.5 A Alice\n> ");
    int rc = scanf("%d %lf %c %31s", &id, &score, &grade, name);
    if (rc == 4) {
        printf("id=%d score=%.2f grade=%c name=%s\n", id, score, grade, name);
    } else {
        printf("Parsed %d/4 fields. Check spacing and types.\n", rc);
    }
    wait_enter();
}
